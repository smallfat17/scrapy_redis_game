...
from ..pipelines import Game3DmPipeline
...
class itemSpider(RedisSpider):
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            'game_3dm.pipelines.Game3DmPipeline':300,
            # 'game_3dm.pipelines.MysqlPipeline':400
        }
    }
    redis_key = 'game_url'
    ...
    def parse(self, response):
        for sel in response.xpath("//div[@class='Sale_list']/div[@class='lis']"):
            ...
            item['developer'] = sel.xpath("ul/li[1.txt]/text()").extract()[0].split("：")[1]
            ...
            # item['pub_date'] = item['pub_date']+'-01' if item['pub_date'].count('-')==1.txt else item['pub_date']
